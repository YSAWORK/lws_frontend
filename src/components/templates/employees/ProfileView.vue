...
<script setup>
  ...
  // отримання даних по ID працівника
  onMounted(async () => {
    try {
      const id = route.params.id;
      console.log('ID з маршруту:', id);
  ...
  // копіювання інформації для документів
  const showTooltip = ref(false);
  const copyInfo = async () => {
    const el = document.getElementById('InfoForDoc');
    if (!el) return;
    ...
    try {
      ...
      setTimeout(() => showTooltip.value = false, 2000); // ховається через 2 сек
    } catch (err) {
      console.error("Помилка копіювання:", err);
    }}

    // Відкриття pdf документів
  const openPDF = (url) => {
    window.open(url, '_blank');
  }

  // редагування профайлу
  const goToAdminProfile = (id) => {
    const backendUrl = import.meta.env.VITE_BACKEND_URL;
    const url = `${backendUrl}/admin/user_app/profile/${id}/change/`;
    window.open(url, '_blank');
  };

  // перехід до списку працівників
  const goToProfileList = (id) => {
